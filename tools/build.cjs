...
for(const name of ['index.html','app.js','style.css','curriculum.js','register-sw.js','library.js','manifest.webmanifest','privacy.html','support.html','icons']) fs.cpSync(path.join(root,name),path.join(out,name),{recursive:true});
const files=[];
...
fs.writeFileSync(path.join(root,'release.json'),JSON.stringify({version,edition:'free-study',questions:64},null,2));
