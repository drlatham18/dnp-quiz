...
assert.equal(qs.length,160);for(const q of qs){assert.equal(q.reviewStatus,'source-checked');assert(q.sources.length);assert(q.rationale);}
for(const f of ['dist/index.html','dist/library.js','dist/stories.js','dist/sw.js','dist/privacy.html','dist/support.html'])assert(fs.existsSync(f),f);
